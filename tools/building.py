...
def aslog_to_file(message, log_file="build_log.txt"):
...
class Library(BuildBase):
    ...
    def objs(self):
        libName = self.name
        aslog_to_file(f"Start building library: {libName}")

        if getattr(self, 'prebuilt_shared', False):
            aslog_to_file(f"Using prebuilt shared library for {libName}")
            return self.source
        
        aslog_to_file(f"Building objects for library: {libName}")
        ...
        aslog_to_file(f"Initial CPPPATH: {CPPPATH}")
        aslog_to_file(f"Initial CPPDEFINES: {CPPDEFINES}")
        aslog_to_file(f"CFLAGS: {CFLAGS}")
        aslog_to_file(f"CPPFLAGS: {CPPFLAGS}")
        
        # Adding includes and configurations
        searched_libs = []
        ...
        try:
            ...
            for src in source[1:]:
                ...
                CPPPATH.append(cfg_path)
            self.source += source
            aslog_to_file(f"Added config path: {cfg_path}, source files: {source}")
        except KeyError:
            self.get_opt_cfg(CPPPATH, libName)
        
        # Process additional library paths
        for name in getattr(self, 'LIBS', []):
            if name in __libraries__:
                try:
                    libInclude = self.RequireCPPPATH(f'${name}')
                    CPPPATH.append(libInclude)
                    aslog_to_file(f"Required include path for {name}: {libInclude}")
                except KeyError:
                    pass
        
        # Sort and log final paths and defines
        CPPPATH = self.sortL(CPPPATH)
        CPPDEFINES = self.sortL(CPPDEFINES)
        aslog_to_file(f"Sorted CPPPATH: {CPPPATH}")
        aslog_to_file(f"Sorted CPPDEFINES: {CPPDEFINES}")
        ...
        for c in self.source:
            ...
            if str(c).endswith('.a') or str(c).endswith('.dll') or str(c).endswith('.so'):
                objs.append(c)
                aslog_to_file(f"Adding prebuilt object file: {c}")
            elif self.is_shared_library():
                objs += env.SharedObject(c, CPPPATH=CPPPATH, CPPDEFINES=CPPDEFINES, CPPFLAGS=CPPFLAGS, CFLAGS=CFLAGS)
                aslog_to_file(f"Building shared object for source: {c}")
            else:
                if 'AS' in env and 'gcc' in env['AS']:
                    if str(c).endswith('.S'):
                        ASFLAGS += [f"-I{x}" for x in CPPPATH]
                        ASFLAGS += [f"-D{x}" for x in CPPDEFINES]
                        objs += env.Object(c, ASFLAGS=ASFLAGS)
                        aslog_to_file(f"Building assembly object for source: {c}")
                        continue
                objs += env.Object(c, CPPPATH=CPPPATH, CPPDEFINES=CPPDEFINES, CPPFLAGS=CPPFLAGS, CFLAGS=CFLAGS)
                aslog_to_file(f"Building object for source: {c}")
        
        aslog_to_file(f"Completed object list for {libName}: {objs}")
        return objs
...
class Application(BuildBase):
    ...
    def build(self):
        ...
        aslog_to_file("start build")
        
        env = self.ensure_env()
        aslog_to_file(f"Environment: {env}")
        
        appName = self.name
        aslog_to_file(f"Application Name: {appName}")
        
        aslog_to_file(f"build application {appName}")
        LIBS = env.get('LIBS', [])
        aslog_to_file(f"Libraries: {LIBS}")
        
        CPPDEFINES = getattr(self, 'CPPDEFINES', []) + list(env.get('CPPDEFINES', []))
        aslog_to_file(f"CPPDEFINES: {CPPDEFINES}")
        ...
        aslog_to_file(f"CPPPATH: {CPPPATH}")
        ...
        for name in self.__libs_order__:
            ...
            aslog_to_file(f"Processing library: {name}")
            
            if lib.is_shared_library():
                aslog_to_file(f"Building shared library {name}")
                objs_ = lib.build()
                objs += objs_
                continue
            
            aslog_to_file(f"Building static library {name}")
            objs_ = lib.objs()
            ...
            if isinstance(env, CustomEnv):
                ...
                thresh = 200
            tooMuch = (len(objs) + len(objs_)) > thresh
            
            libObjs = []
            for obj in objs_:
                obj_str = str(obj)
                aslog_to_file(f"Object file: {obj_str}")
                
                if obj_str.endswith('.a'):
                    libName = os.path.basename(obj_str)[3:-2]
                    p = os.path.dirname(obj_str)
                    if p not in LIBPATH:
                        LIBPATH.append(p)
                    if libName in ['Dcm', 'Com']:
                        ...
                        LIBS.append(libName)
                elif obj_str.endswith('.dll'):
                    libName = os.path.basename(obj_str)[:-4]
                    p = os.path.dirname(obj_str)
                    if p not in LIBPATH:
                        LIBPATH.append(p)
                    LIBS.append(libName)
            ...
            if libObjs:
                libName = name.split(':')[0] if ':' in name else name
        ...
        target = env.Program(appName, objs, CPPPATH=CPPPATH, CPPDEFINES=CPPDEFINES, CPPFLAGS=CPPFLAGS, LIBS=LIBS, LINKFLAGS=LINKFLAGS, LIBPATH=LIBPATH)
        aslog_to_file(f"Program target: {target}")
        
        if 'S19' in env:
            BUILD_DIR = os.path.dirname(target[0].get_abspath())
            action = env['S19'].format(target[0].get_abspath(), f"{BUILD_DIR}/{appName}.s19")
            env.AddPostAction(target, action)
            aslog_to_file(f"S19 action: {action}")
        
        if 'ELFSIZE' in env:
            BUILD_DIR = os.path.dirname(target[0].get_abspath())
            action = env['ELFSIZE'].format(target[0].get_abspath())
            env.AddPostAction(target, action)
            aslog_to_file(f"ELFSIZE action: {action}")
            
            readelf = f"echo python {RootDir}/tools/utils/readelf.py -i {target[0].get_abspath()} -o {BUILD_DIR}/{appName}.json"
            env.AddPostAction(target, readelf)
            aslog_to_file(f"Readelf action: {readelf}")
        
        for action in getattr(self, '__post_actions__', []):
            env.AddPostAction(target, action)
            aslog_to_file(f"Post action: {action}")
        
        self.do_install(target)
        aslog_to_file(f"Build completed for {appName}")
